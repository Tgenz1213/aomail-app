...
def get_batch_emails(request: HttpRequest) -> Response:
    try:
        ...
        if not email_ids or not isinstance(email_ids, list):
            return Response({"error": "Invalid or missing email_ids"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if not first_email:
            return Response({"error": "No matching email found"}, status=status.HTTP_400_BAD_REQUEST)
        
        social_api = first_email.social_api
        
        if not social_api:
            return Response({"error": "No linked email account found"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if type_api == "google":
            ...
            return Response({"error": "Unsupported email provider"}, status=status.HTTP_400_BAD_REQUEST)
