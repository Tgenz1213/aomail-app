...
def create_signature(request: HttpRequest) -> Response:
    ...
    try:
        ...
        try:
            ...
            return Response(
                {"error": "SocialAPI not found."}, status=status.HTTP_404_NOT_FOUND
            )
...
def update_signature(request: HttpRequest) -> Response:
    ...
    try:
        ...
        try:
            ...
            return Response(
                {"error": "Signature not found."}, status=status.HTTP_404_NOT_FOUND
            )
...
def delete_signature(request: HttpRequest, signature_id: int) -> Response:
    ...
    try:
        ...
        signature.delete()
        return Response(
            {"message": "Signature deleted successfully."}, status=status.HTTP_200_OK
        )
    except Signature.DoesNotExist:
        return Response(
            {"error": "Signature not found."}, status=status.HTTP_404_NOT_FOUND
