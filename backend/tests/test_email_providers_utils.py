...
from aomail.models import Category, Email, Rule, Sender, SocialAPI, Statistics
...
def rules(user: User):
    category, _ = Category.objects.get_or_create(
        user=user, description="Default category", name=DEFAULT_CATEGORY
    )
...
def test_verify_condition(processed_email: dict, rules: list[Rule]):
    ...
    processed_email["email_processed"]["category"] = "Other Category"
    ...
    # Test transfer recipients condition
...
@pytest.fixture
def test_save_email_to_db(processed_email: dict, social_api: SocialAPI, sender: Sender):
    email_entry = save_email_to_db(processed_email, social_api.user, sender)
    ...
    assert email_entry.category == DEFAULT_CATEGORY
    assert email_entry.user == social_api.user
