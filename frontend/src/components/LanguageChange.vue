      ...
      <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <ListboxOptions class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
          <ListboxOption as="template" v-for="language in languages" :key="language.key" :value="language">
            <li :class="{ 'bg-gray-800 text-white': active, 'text-gray-900': !active, 'relative cursor-default select-none py-2 pl-3 pr-9': true }">
              <span :class="{ 'font-semibold': selected, 'font-normal': !selected, 'block truncate': true }">{{ $t(language.i18nKey) }}</span>
              <span v-if="selected" :class="{ 'text-white': active, 'text-gray-500': !active, 'absolute inset-y-0 right-0 flex items-center pr-4': true }">
...
const selectedLanguage = ref(languages.value[0]); // Default selection
const emit = defineEmits(['language-updated']);

const updateLanguageSelection = (newLanguage) => {
  selectedLanguage.value = newLanguage;
  emit('language-updated', newLanguage);
};
</script>