    ...
    <div class="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex items-center justify-center" v-if="isOpen">
      <div class="bg-white rounded-lg relative">
        ...
        <div class="flex flex-col px-8">
          <div class="mt-4 mb-8">
            <div class="sm:hidden">
              <label for="tabs" class="sr-only">{{ $t('SeeMail_vue.select_tab') }} </label>
              <!-- Use an "onChange" listener to redirect the user to the selected tab URL. -->
              <select id="tabs" name="tabs"
                class="block w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
                <option selected>{{ $t('SeeMail_vue.mail_resume') }} </option>
                <option>{{ $t('SeeMail_vue.full_mail') }} </option>
              </select>
            </div>
