<template>
  <div v-if="visible" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
    <div class="bg-white rounded-lg shadow-lg w-11/12 md:w-2/3 lg:w-1/2 xl:w-1/3 p-6 relative">
      <h2 class="text-xl font-semibold mb-4">Create Email Signature</h2>
      <form @submit.prevent="handleSubmit">
        <div class="mb-4">
          <label class="block text-gray-700 mb-2">Signature Content:</label>
          <div
            ref="editorRef"
            contenteditable="true"
            class="w-full border rounded p-2 min-h-[150px] overflow-auto"
            placeholder="Type your signature or paste/drop an image here"
            @input="handleInput"
            @paste="handlePaste"
            @dragover.prevent
            @drop="handleDrop"
...
const handleSubmit = async () => {
  if (!props.selectedEmailId) {
    displayPopup?.("error", "Error", "No email selected.");
    return;
  }

  /* Not needed if (!signatureContent.value.trim()) {
    displayPopup?.("error", "Error", "Please provide signature content.");
    return;
  } */
  ...
  try {
    ...
    if (response.success) {
      displayPopup?.("success", "Success", "Signature created successfully.");
      emit("created", response.data);
      emit("close");
    } else {
      displayPopup?.("error", "Error", response.error || "Failed to create signature.");
    }
  } catch (error) {
    displayPopup?.("error", "Error", "An unexpected error occurred.");
  }
};
...
const handleEmptySignature = async () => {
  ...
  try {
    ...
    console.error("Failed to create empty signature:", error);
  }
