<template>
    <div class="flex items-stretch border-l h-12 2xl:h-14 space-x-2 px-4">
        <div v-for="(filter, index) in visibleFilters" :key="index" class="relative group flex items-center">
            <div class="flex items-center h-full relative group hover:bg-gray-100 transition-all duration-200">
                <button
                ...
                >
                    <span
                        class="relative z-10"
                        :class="
                            selectedFilter?.name === filter.name ? 'text-black font-medium' : 'text-black font-normal'
                        "
                    >
                        {{ filter.name }}
                    </span>
        ...
        <button @click="openNewFilterModal" class="px-2 py-1 text-gray-600 hover:bg-gray-100 focus:outline-none">
            +
        </button>
    </div>
</template>
...
watch(selectedCategory, async (newCategory, _oldCategory) => {
