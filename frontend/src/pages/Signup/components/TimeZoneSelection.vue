...
const timezones = ref<string[]>(moment.tz.names());
const initialLoadCount = Math.ceil(timezones.value.length * 0.25);
const visibleTimezones = ref<string[]>(timezones.value.slice(0, initialLoadCount));

const storedTimeZone = localStorage.getItem("timezone") || "UTC";
const selectedTimeZone = ref<string>(storedTimeZone);
...
const updateTimeZoneSelection = (newTimeZone: string) => {
