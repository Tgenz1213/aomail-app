            ...
            <div class="flex h-full w-full">
                ...
                <div class="flex-1">
                    ...
                    <div class="flex flex-col h-full w-full">
                        ...
                        <div v-if="!isEmptyTopic()" class="w-full flex relative border-b border-black shadow-sm border-opacity-10 shadow-[-1px_0_0_0_rgba(0,0,0,0.05)]">
                            <!-- Search bar -->
                            <div class="flex-grow -z-10">
                                <SearchbarV2 @input="updateSearchQuery" height="3rem" :showBorder="false" />
                            </div>

                            <!-- Filter activation button (to the right of the search bar) -->
                            <button @click="toggleFilterSection" class="absolute p-2 rounded-md hover:bg-gray-100 focus:outline-none mr-2">
...
function totalEmailsInCategoryNotRead(categoryName) {
    let totalCount = 0;
    console.log("CHECK", emails.value);
    const category = emails.value[categoryName];
...
const groupedEmailsByCategoryAndDate = (category) => {
 ...
 console.log("DEBUG : ", sortedGrouped)
...
async function fetchEmailDetails(emailIds) {
  ...
  try {
    ...
    for (const category in result.data) {
      for (const priority in result.data[category]) {
        if (!transformedResult[priority]) {
          transformedResult[priority] = [];
        }
        transformedResult[priority].push(...result.data[category][priority]);
      }
    }*/
...
methods: {
    updateSearchQuery(event) {
      console.log("Updating search query:", event.target.value)
    },
}
}
</script>