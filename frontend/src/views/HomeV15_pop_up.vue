...
function scrollToBottom() {
  $nextTick(() => {
    const element = $refs.scrollableDiv;
...
function toggleHiddenParagraph(index) {
  ...
  console.log('parentElement: ', $refs['parentElement' + index])
  console.log("Test: ", $refs['parentElement' + index][0].children)

  showHiddenParagraphs[index] = !showHiddenParagraphs[index];
  $nextTick(() => {
    if (showHiddenParagraphs[index] && !animationTriggered[index]) {
      const parentElement = $refs['parentElement' + index][0];
