              ...
              <div v-if="nbr_reply_answer > 0">
                <ul role="list" class="flex flex-col w-full h-full rounded-xl">
                  <li v-if="emails['Important'] && emails['Important'].length > 0"
                    class="py-10 px-8 mx-4 my-4 rounded-xl bg-red-100 bg-opacity-50 hover:border border-red-700 border-opacity-20">
                        ...
                        <div class="overflow-hidden border-l-4 border-red-500  hover:rounded-l-xl dark:border-red-300">
                          <ul role="list" class="divide-y divide-gray-200 dark:divide-white">
                            <li v-for="item in emails['Important']" :key="item.id"
                              class="px-6 py-4 hover:bg-opacity-70 dark:hover:bg-red-500 dark:hover:bg-opacity-100 grid grid-cols-10 gap-4 items-center"
                              ...
                              <div class="col-span-8">
                                <div class="flex-auto">
                                  ...
                                  <p class="mt-1 text-md text-gray-700 leading-relaxed dark:text-red-50">{{
                                    item.description }}</p>
                                </div>
                                <ul v-show="showHiddenParagraphs[item.id]" role="list" class="text-black text-sm/6 pt-2"
                                  :ref="'parentElement' + item.id">
                                  <li v-for="detail in item.details" :key="detail.id" class="pl-8 my-2"
                  ...
                  <li v-if="emails['Information'] && emails['Information'].length > 0"
                    class="py-10 px-8 mx-4 my-4 rounded-xl bg-blue-100 bg-opacity-50 hover:border border-blue-700 border-opacity-20 w-full">
                      ...
                      <li v-for="item in emails['Information']" :key="item.id"
                        ...
                        <div class="col-span-8" @click="toggleHiddenParagraph(item.id)">
                          <div class="flex-auto group">
                            ...
                            <p class="mt-1 text-md text-gray-700 leading-relaxed dark:text-blue-50">{{ item.description }}
                            </p>
                          </div>
                          <ul v-show="showHiddenParagraphs[item.id]" role="list" class="text-black text-sm/6 pt-2"
                            :ref="'parentElement' + item.id">
                            <!-- Potential design update : bg-white shadow rounded-xl -->
                            <li v-for="detail in item.details" :key="detail.id" class="pl-8" :ref="'hiddenText' + item.id"
                              ...
                              <div v-show="hoveredItemId === item.id" class="group action-buttons">
                                <div class="relative group">
                                  ...
                                  <button type="button"
                                    ...
                                    <check-icon @click="markEmailAsRead(item.id)"
                                  ...
                                  <Menu as="div" class="relative inline-block text-left">
                                    ...
                                    <transition enter-active-class="transition ease-out duration-100"
                                      ...
                                      <MenuItems v-show="isMenuOpen"
                                        ...
                                        <div class="py-1">
                                          <MenuItem v-slot="{ active }">
                                          <a @click.prevent="markEmailReplyLater(item.id)" href="#"
                                            :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'block px-4 py-1 text-sm']">
                                            <span class="flex gap-x-2 items-center">
                  ...
                  <div v-if="emails['Useless'] && emails['Useless'].length > 0"
                    class="flex-1 mx-4 my-4 rounded-xl bg-gray-100 hover:border border-gray-700 border-opacity-20 w-full">
                    <li class="py-10 px-8"> <!-- ring-1 ring-red-700 ring-opacity-20 -->
                      ...
                      <div class="flex">
                        <div class="flex">
                          <span
                            class="inline-flex h-14 w-14 items-center justify-center rounded-full bg-gray-400 dark:bg-red-200">
                            <span class="text-lg font-medium leading-none text-white dark:text-red-400">AO</span>
                          </span>
                          <!--<ChatBubbleOvalLeftEllipsisIcon class="w-6 h-6 text-red-500" />-->
                        </div>
                        <div class="ml-6 ">
                          <div class="overflow-hidden border-l-4 hover:rounded-l-xl border-gray-500 dark:border-red-300"
                            @click="toggleHiddenParagraph(3)">
                            <ul role="list" class="divide-y divide-gray-200 dark:divide-white">
                              <li class="px-6 py-4 dark:bg-red-500 hover:bg-opacity-70 dark:hover:bg-opacity-100">
                                <div class="flex-auto">
                                  Vous avez reçu <span
                                    class="font-semibold text-gray-800 dark:text-white hover:text-gray-700">4</span> mails
                                  inutiles. Cliquez pour voir.
                                </div>
                                <ul role="list" v-show="showHiddenParagraphs[3]" class="text-gray-800 text-sm/6 pt-2"
                                  ref="parentElement3">
                                  <li class="pl-8" ref="hiddenText3"
                                    data-text="- Fabien Huet est en vacances dans les Alpes."></li>
                                  <li class="pl-8" ref="hiddenText3"
                                    data-text="- Il demande à recevoir les différentes informations pour le BP afin d'avancer rapidement sur le sujet.">
                                  </li>
                                  <li class="pl-8" ref="hiddenText3"
                                    data-text="- Il souhaite également prendre rendez-vous avec son expert-comptable d'ici la fin du mois.">
                                  </li>
                                </ul>
                              </li>
                              <!-- More items... -->
